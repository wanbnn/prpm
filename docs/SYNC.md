...
Use `prpm sync` when the virtual environment must match the current lockfile exactly:
...
`sync` requires a current lockfile. It first installs the exact locked graph using the same frozen-install path as `prpm install --frozen`. Only after that succeeds does it inspect `.venv` and uninstall packages that are not part of the selected lock scope.
...
For CI or deployment environments that must contain no undeclared Python packages, use:
